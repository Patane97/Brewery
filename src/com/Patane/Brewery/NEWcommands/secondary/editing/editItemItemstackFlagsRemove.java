...
public class editItemItemstackFlagsRemove extends editItemItemstackFlags {
	...
	public boolean execute(CommandSender sender, String[] args, Object... objects) {
		...
		String successMsg = "&aRemoved Item Flag from &7"+brItem.getName()+"&a. Hover for details!";
		...
		if(args[0].equalsIgnoreCase("all")) {
			...
			successMsg = "&aRemoved all Item Flags from &7"+brItem.getName()+"&a. Hover for details!";
			// Printing all the flags being removed in 'remove' format
			successHoverText = "&2Item Flags:" + StringsUtil.formatter(s -> "\n&c> &8&m"+s[0], StringsUtil.enumValueStrings(ItemsUtil.getFlags(currentItem).toArray(new ItemFlag[0])));
		...
		// If specific flag has been provided
			...
			if(!ItemsUtil.getFlags(currentItem).contains(itemFlag)){
				...
				Messenger.send(sender, StringsUtil.hoverText("&7"+brItem.getName()+"&e does not have that Item Flag. Hover to view which ones it does have!", successHoverText));
				return true;
			}
